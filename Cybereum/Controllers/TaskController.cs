    ...
    public class TaskController : Controller
    {
        ...
        public async Task<JsonResult> tasklist(string activityid)
        {
            ...
            try
            {
                ...
                //{
                var gremlinScript = "g.V().has('task','assignedto','" + pmuserid + "').project('id','taskname','startdate','enddate','durations','tasktype','taskstatus','assignedto','createdby','createdusername','createdon','activityid','progress')" +
                ...
                try
                {
                    ...
                    foreach (var result in results)
                    {
                        ...
                        gremlinScript = "g.V().has('activity','id','" + result["activityid"] + "').project('activityname','projectid').by(values('activityname')).by(values('projectid'))";
                        ...
                        foreach (var result1 in results1)
                        {
                            ...
                            gremlinScript = "g.V().has('project','id','" + result1["projectid"] + "').project('projectname').by(values('projectname'))";
                            var projresult = IGUtilities.ExecuteGremlinScript(gremlinScript);
                            foreach (var res in projresult)
                            {
                                task.projectname = res["projectname"];
                            }
                        }
                        ...
                        task.taskstatus = Convert.ToInt16(result["taskstatus"]);
                        task.progress = Convert.ToInt16(result["progress"]);

                        list.Add(task);
                    }

                    //string pList = JsonConvert.SerializeObject(results);
                    //list = JsonConvert.DeserializeObject<List<ProjectTask>>(pList);
                    ...
                    //var joinedData = from m in list
                    //                 join r in db.tbl_user on m.assignedto equals r.userid
                    //                 select new
                    //                 {
                    //                     taskid = m.taskid,
                    //                     taskname = m.taskname,
                    ...
                    //                     assignedusername = r.firstname + " " + r.lastname,
        ...
        public async Task<ProjectTask> gettaskbyid(string id)
        {
            ...
            try
            {
                ...
                var gremlinScript = "g.V().has('task','id','" + id + "').project('id','taskname','startdate','enddate','durations','tasktype','taskstatus','assignedto','createdby','createdusername','createdon','activityid','progress')" +
                    ".by(id()).by(values('taskname')).by(values('startdate')).by(values('enddate')).by(values('durations')).by(values('tasktype')).by(values('taskstatus')).by(values('assignedto')).by(values('createdby')).by(values('createdusername')).by(values('createdon')).by(values('activityid')).by(values('progress'))";
                try
                {
                    var results = IGUtilities.ExecuteGremlinScript(gremlinScript);
                    foreach (var result in results)
                    {
                        ...
                        projecttask.progress = Convert.ToInt16(result["progress"]);
        ...
        public ActionResult AddEditTask([Bind(Include = "taskid,taskname,startdate,enddate,durations,tasktype,taskstatus,assignedto,createdby,createdon,activityid,progress")] ProjectTask tbl_task)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var enddate = CheckActivitydates(tbl_task.activityid, tbl_task.startdate, tbl_task.enddate);
                ...
                if (tbl_task.taskid == null)
                {
                    ...
                    string gremlinScript = $"g.addV('task').property('pk', '{tbl_task.taskname}')" +
                            ...
                            $".property('progress', '{tbl_task.progress}')" +
                    ...
                    gremlinScript = $"\ng.V('{tbl_task.taskid}').addE('contains').to(g.V('{tbl_task.activityid}'))";
                    // Execute the Gremlin script
                    result = IGUtilities.ExecuteGremlinScript(gremlinScript);
                    message = "Gremlin script executed successfully";
                ...
                {
                    if (tbl_task.progress == 100)
                    {
                        tbl_task.taskstatus = (int)TaskSubTaskStatus.Completed;
                    }
                    else if (tbl_task.taskstatus == (int)TaskSubTaskStatus.Completed)
                    {
                        tbl_task.progress = 100;
                    }
                ...
                if (tbl_task.progress > 0)
                {
                    IGUtilities.updatetaskprogress(tbl_task.taskid);
                    IGUtilities.updateactivityprogress(tbl_task.activityid);
                }
